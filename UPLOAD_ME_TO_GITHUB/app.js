...
function setTransactionType(type) {
    ...
    const btnInt = document.getElementById('toggle-internal');
    const btnExt = document.getElementById('toggle-external');

    if (type.toLowerCase() === 'internal') {
        btnInt.className = 'flex-1 py-1 rounded-lg text-xs font-bold transition-all bg-blue-600 text-white shadow-lg';
        btnExt.className = 'flex-1 py-1 rounded-lg text-xs font-bold transition-all text-slate-400 hover:text-white';
    } else {
        btnInt.className = 'flex-1 py-1 rounded-lg text-xs font-bold transition-all text-slate-400 hover:text-white';
        btnExt.className = 'flex-1 py-1 rounded-lg text-xs font-bold transition-all bg-blue-600 text-white shadow-lg';
    }
}
...
function checkCategory() {
    ...
    if (cat === 'Spare parts' || cat === 'Services') {
...
function renderChart(categoryTotals, totalSpent) {
    ...
    const colorMap = {
        'Spare parts (internal)': '#8B5CF6',
        'Spare parts (External)': '#A78BFA',
        'Services (internal)': '#F97316',
        'Services (External)': '#FB923C',
    ...
    if (hasData) {
        for (let [cat, amt] of Object.entries(categoryTotals)) {
            if (amt > 0) {
                ...
                // Robust color lookup (case insensitive for suffixes)
                let lookupKey = cat;
                if (cat.toLowerCase().includes('services (internal)')) lookupKey = 'Services (internal)';
                if (cat.toLowerCase().includes('services (external)')) lookupKey = 'Services (External)';
                if (cat.toLowerCase().includes('spare parts (internal)')) lookupKey = 'Spare parts (internal)';
                if (cat.toLowerCase().includes('spare parts (external)')) lookupKey = 'Spare parts (External)';

                bgColors.push(colorMap[lookupKey] || colorMap[cat] || '#CBD5E1');
            }
        }
    } else {
        labels = ['No Data'];
...
async function exportToExcel() {
    try {
        ...
        // Headers
        const headers = [
            'Date', 'Supplier', 'Vehicle Number', 'Bill number', 'Description',
            'Total', 'Spare parts (internal)', 'Spare parts (External)', 'Services (Internal)',
            'Services (External)', 'Mechanical maintenance', 'Transport', 'Office items',
            'Employee welfare', 'Other', 'Balance'
        ];
        ...
        let sumAmount = 0;
        let sums = {
            'Spare parts (internal)': 0, 'Spare parts (External)': 0,
            'Services (internal)': 0, 'Services (External)': 0,
            'Mechanical maintenance': 0, 'Transport': 0,
            'Office items': 0, 'Employee welfare': 0, 'Other': 0
        };
        ...
        allExps.forEach(exp => {
            ...
            // Category Mapping/Normalizing
            let cat = exp.category;
            if (cat === 'Spares') cat = 'Spare parts (External)';
            if (cat === 'External Service') cat = 'Services (External)';
            if (cat === 'Mechanical Maintenance') cat = 'Mechanical maintenance';
            if (cat === 'Office Items') cat = 'Office items';
            if (cat === 'Employee Welfare') cat = 'Employee welfare';
            if (cat === 'Courier' || cat === 'Others') cat = 'Other';
            // Correct the toggle outputs if they deviate from display strings
            if (cat === 'Spare parts (internal)') cat = 'Spare parts (internal)';
            if (cat === 'Services (internal)') cat = 'Services (internal)';
            if (cat === 'Services (Internal)') cat = 'Services (internal)';

            if (cat === 'Services (External)') cat = 'Services (External)';

            if (sums[cat] !== undefined) sums[cat] += exp.amount;
            ...
            const rowData = {
                ...
                amount: exp.amount,
                sp_int: cat === 'Spare parts (internal)' ? exp.amount : '',
                sp_ext: cat === 'Spare parts (External)' ? exp.amount : '',
                sv_int: cat === 'Services (Internal)' ? exp.amount : '',
                sv_ext: cat === 'Services (External)' ? exp.amount : '',
                mech_maint: cat === 'Mechanical maintenance' ? exp.amount : '',
                trans: cat === 'Transport' ? exp.amount : '',
                office: cat === 'Office items' ? exp.amount : '',
                welfare: cat === 'Employee welfare' ? exp.amount : '',
                other: cat === 'Other' ? exp.amount : '',
                balance: currentBalance
            };
        ...
        const totalRow = sheet.addRow({
            ...
            amount: sumAmount,
            sp_int: sums['Spare parts (internal)'],
            sp_ext: sums['Spare parts (External)'],
            sv_int: sums['Services (Internal)'],
            sv_ext: sums['Services (External)'],
            mech_maint: sums['Mechanical maintenance'],
            trans: sums['Transport'],
        ...
        const summaryData = [
            { label: 'Spare parts (internal) -', value: sums['Spare parts (internal)'] },
            { label: 'Spare parts (External) -', value: sums['Spare parts (External)'] },
            { label: 'Services (Internal) -', value: sums['Services (Internal)'] },
            { label: 'Services (External) -', value: sums['Services (External)'] },
            { label: 'Mechanical maintainance -', value: sums['Mechanical maintenance'] },
            { label: 'Transport -', value: sums['Transport'] },
            { label: 'Office items -', value: sums['Office items'] },
            { label: 'Employee welfare -', value: sums['Employee welfare'] },
            { label: 'Other -', value: sums['Other'] },
            { label: 'Total expenses -', value: sumAmount, isTotal: true }
        ];

        summaryData.forEach(item => {
            const row = sheet.addRow([]);
            sheet.mergeCells(row.number, 13, row.number, 15);
            const labelCell = row.getCell(13);
            const valueCell = row.getCell(16);
